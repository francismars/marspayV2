...
import { appendPaymentToLNURLPFromId, getGameInfoFromID, getIDFromLNURLP, getLNURLPsFromID, getPlayerInfoFromIDToGame, getPlayerValueFromGameSession, setPlayerInfoInGameByID } from "../socket/sessionManager";
...
router.post("/", (req: Request, res: Response) => {
  ...
  console.log(`${dateNow()} [${sessionID}] Paid LNURLp ${reqLNURLP} with ${amount} sats and note ${comment}.`);
  ...
  if(LNURLPs){
    ...
    appendPaymentToLNURLPFromId({"amount":amount, "note":comment}, reqLNURLP, sessionID)
    const value = getPlayerValueFromGameSession(sessionID, playerRole) ?? 0
    const playerInfo: PlayerInfo = {"name": comment ?? playerRole, "value": value + amount}
    const gameMode = "P2P" as GameMode
    setPlayerInfoInGameByID(sessionID, playerRole, playerInfo, gameMode)
  } 
  //console.log(JSON.stringify(getLNURLPsFromID(sessionID)))
  console.log(JSON.stringify(getGameInfoFromID(sessionID)))
