/**
 * Compact ON-disk replay: static header + minimal per-frame payload + gzip.
 * Live gameplay still uses full OnlineRoomSnapshot[] in memory.
 */
import { gzipSync, gunzipSync } from 'zlib';
import type {
  Coinbase,
  Direction,
  OnlineAuthoritativeState,
  OnlineHudState,
  PlayerId,
  PointChange,
} from '../game/onlineEngine';
import { getCaptureLabel, getOnlineHudState } from '../game/onlineEngine';
import type { OnlineRoomPhase, OnlineRoomSnapshot } from '../types/online';

const COMPACT_FORMAT = 'compact-v2' as const;

const PHASE_LIST: OnlineRoomPhase[] = [
  'lobby',
  'playing',
  'postgame',
  'finished',
  'cancelled',
];

const DIR_LIST: Direction[] = ['', 'Up', 'Down', 'Left', 'Right'];

function dirToNum(d: Direction): number {
  const i = DIR_LIST.indexOf(d);
  return i >= 0 ? i : 0;
}

function numToDir(n: number): Direction {
  return DIR_LIST[n] ?? '';
}

function flattenBody(body: [number, number][]): number[] {
  const out: number[] = [];
  for (const [x, y] of body) {
    out.push(x, y);
  }
  return out;
}

function unflattenBody(flat: number[]): [number, number][] {
  const out: [number, number][] = [];
  for (let i = 0; i + 1 < flat.length; i += 2) {
    out.push([flat[i]!, flat[i + 1]!]);
  }
  return out;
}

export interface CompactReplayHeader {
  cols: number;
  rows: number;
  p1Name: string;
  p2Name: string;
  initialScore: [number, number];
  meta: OnlineAuthoritativeState['meta'];
}

interface EncodedFrame {
  t: number;
  p: number;
  p1: { h: [number, number]; b: number[]; d: number; w: number };
  p2: { h: [number, number]; b: number[]; d: number; w: number };
  cb: Array<[number, number] | [number, number, number]>;
  f: number;
  ct: number;
  wp: 0 | 1 | 2;
  wn: string;
  sc: [number, number];
  tp: number;
  pc: Array<{
    p: 1 | 2;
    v: number;
    x1: number;
    y1: number;
    x2: number;
    y2: number;
    o1: number;
    o2: number;
    a: number;
  }>;
}

function packFlags(st: OnlineAuthoritativeState): number {
  let f = 0;
  if (st.gameStarted) {
    f |= 1;
  }
  if (st.gameEnded) {
    f |= 2;
  }
  if (st.countdownStart) {
    f |= 4;
  }
  if (st.sentWinner) {
    f |= 8;
  }
  return f;
}

function applyFlags(f: number, st: OnlineAuthoritativeState): void {
  st.gameStarted = (f & 1) !== 0;
  st.gameEnded = (f & 2) !== 0;
  st.countdownStart = (f & 4) !== 0;
  st.sentWinner = (f & 8) !== 0;
}

function winnerToNum(w: PlayerId | null): 0 | 1 | 2 {
  if (w === null) {
    return 0;
  }
  return w === 'P1' ? 1 : 2;
}

function numToWinner(n: 0 | 1 | 2): PlayerId | null {
  if (n === 0) {
    return null;
  }
  return n === 1 ? 'P1' : 'P2';
}

function encodeCoinbases(cbs: Coinbase[]): EncodedFrame['cb'] {
  return cbs.map((cb) => {
    if (cb.reward != null) {
      return [cb.pos[0], cb.pos[1], cb.reward] as [number, number, number];
    }
    return [cb.pos[0], cb.pos[1]] as [number, number];
  });
}

function decodeCoinbases(cb: EncodedFrame['cb']): Coinbase[] {
  return cb.map((row) => {
    if (row.length === 3) {
      return { pos: [row[0], row[1]], reward: row[2] as Coinbase['reward'] };
    }
    return { pos: [row[0], row[1]] };
  });
}

function encodePointChanges(pc: PointChange[]): EncodedFrame['pc'] {
  return pc.map((c) => ({
    p: c.player === 'P1' ? (1 as const) : (2 as const),
    v: c.value,
    x1: c.p1Pos[0],
    y1: c.p1Pos[1],
    x2: c.p2Pos[0],
    y2: c.p2Pos[1],
    o1: c.p1YOffsetPx,
    o2: c.p2YOffsetPx,
    a: c.alpha,
  }));
}

function decodePointChanges(pc: EncodedFrame['pc']): PointChange[] {
  return pc.map((c) => ({
    player: c.p === 1 ? 'P1' : 'P2',
    value: c.v,
    p1Pos: [c.x1, c.y1],
    p2Pos: [c.x2, c.y2],
    p1YOffsetPx: c.o1,
    p2YOffsetPx: c.o2,
    alpha: c.a,
  }));
}

function encodeFrame(snap: OnlineRoomSnapshot): EncodedFrame {
  const st = snap.state;
  const ph = PHASE_LIST.indexOf(snap.phase);
  return {
    t: snap.tick,
    p: ph >= 0 ? ph : 0,
    p1: {
      h: [st.p1.head[0], st.p1.head[1]],
      b: flattenBody(st.p1.body),
      d: dirToNum(st.p1.dir),
      w: dirToNum(st.p1.dirWanted),
    },
    p2: {
      h: [st.p2.head[0], st.p2.head[1]],
      b: flattenBody(st.p2.body),
      d: dirToNum(st.p2.dir),
      w: dirToNum(st.p2.dirWanted),
    },
    cb: encodeCoinbases(st.coinbases),
    f: packFlags(st),
    ct: st.countdownTicks,
    wp: winnerToNum(st.winnerPlayer),
    wn: st.winnerName,
    sc: [st.score[0], st.score[1]],
    tp: st.totalPoints,
    pc: encodePointChanges(st.pointChanges),
  };
}

function buildState(ef: EncodedFrame, header: CompactReplayHeader): OnlineAuthoritativeState {
  const p1Body = unflattenBody(ef.p1.b);
  const p2Body = unflattenBody(ef.p2.b);
  const st: OnlineAuthoritativeState = {
    cols: header.cols,
    rows: header.rows,
    p1: {
      head: [ef.p1.h[0], ef.p1.h[1]],
      body: p1Body,
      dir: numToDir(ef.p1.d),
      dirWanted: numToDir(ef.p1.w),
    },
    p2: {
      head: [ef.p2.h[0], ef.p2.h[1]],
      body: p2Body,
      dir: numToDir(ef.p2.d),
      dirWanted: numToDir(ef.p2.w),
    },
    coinbases: decodeCoinbases(ef.cb),
    gameStarted: false,
    gameEnded: false,
    countdownStart: false,
    sentWinner: false,
    countdownTicks: ef.ct,
    winnerPlayer: numToWinner(ef.wp),
    winnerName: ef.wn,
    score: [ef.sc[0], ef.sc[1]],
    totalPoints: ef.tp,
    pointChanges: decodePointChanges(ef.pc),
    p1Name: header.p1Name,
    p2Name: header.p2Name,
    meta: header.meta,
    initialScore: [header.initialScore[0], header.initialScore[1]],
    currentCaptureP1: getCaptureLabel(p1Body.length),
    currentCaptureP2: getCaptureLabel(p2Body.length),
  };
  applyFlags(ef.f, st);
  return st;
}

function decodeFrame(ef: EncodedFrame, header: CompactReplayHeader): OnlineRoomSnapshot {
  const state = buildState(ef, header);
  const hud: OnlineHudState = getOnlineHudState(state);
  return {
    tick: ef.t,
    phase: PHASE_LIST[ef.p] ?? 'lobby',
    state,
    hud,
  };
}

function headerFromFirstFrame(first: OnlineRoomSnapshot): CompactReplayHeader {
  const st = first.state;
  return {
    cols: st.cols,
    rows: st.rows,
    p1Name: st.p1Name,
    p2Name: st.p2Name,
    initialScore: [st.initialScore[0], st.initialScore[1]],
    meta: { ...st.meta },
  };
}

/** Written to disk archives (gzip JSON of header + thin frames). */
export type PackedReplay = {
  format: typeof COMPACT_FORMAT;
  tickMs: number;
  gzipBase64: string;
  frameCount: number;
};

/** Persist: compact + gzip. Empty frames → zero-length payload. */
export function packReplayForArchive(
  frames: OnlineRoomSnapshot[],
  tickMs: number
): PackedReplay {
  if (frames.length === 0) {
    return { format: COMPACT_FORMAT, tickMs, gzipBase64: '', frameCount: 0 };
  }
  const header = headerFromFirstFrame(frames[0]!);
  const encoded = frames.map(encodeFrame);
  const json = JSON.stringify({ h: header, f: encoded });
  const gz = gzipSync(Buffer.from(json, 'utf8'), { level: 9 });
  return {
    format: COMPACT_FORMAT,
    tickMs,
    gzipBase64: gz.toString('base64'),
    frameCount: frames.length,
  };
}

/** Restore full snapshots for clients (legacy full JSON or compact-v2). */
export function expandReplayPayload(
  replay: unknown
): { tickMs: number; frames: OnlineRoomSnapshot[] } | null {
  if (!replay || typeof replay !== 'object') {
    return null;
  }
  const r = replay as Record<string, unknown>;
  const tickMs = r.tickMs;
  if (typeof tickMs !== 'number') {
    return null;
  }
  if (r.format === COMPACT_FORMAT) {
    const b64 = r.gzipBase64;
    if (typeof b64 !== 'string') {
      return null;
    }
    if (b64 === '') {
      return { tickMs, frames: [] };
    }
    try {
      const raw = gunzipSync(Buffer.from(b64, 'base64')).toString('utf8');
      const data = JSON.parse(raw) as { h: CompactReplayHeader; f: EncodedFrame[] };
      if (!data.h || !Array.isArray(data.f)) {
        return null;
      }
      const frames = data.f.map((ef) => decodeFrame(ef, data.h));
      return { tickMs, frames };
    } catch {
      return null;
    }
  }
  if (Array.isArray(r.frames)) {
    return { tickMs, frames: r.frames as OnlineRoomSnapshot[] };
  }
  return null;
}

/** Frame count for archive replay union (compact or legacy full frames). */
export function replayFrameCount(
  replay: PackedReplay | { tickMs: number; frames: OnlineRoomSnapshot[] }
): number {
  if ('format' in replay && replay.format === COMPACT_FORMAT) {
    return replay.frameCount;
  }
  if ('frames' in replay && Array.isArray(replay.frames)) {
    return replay.frames.length;
  }
  return 0;
}
