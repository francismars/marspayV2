...
import { isEliminationMode, isFfaPlayerAlive, isFfaMode, is2v1Mode, multiplayerPlayerCount, get2v1TeamCapturePercent } from './ffa';
import { gameRandom } from './runRng';
import type {
  ActivePowerUp,
  Coinbase,
  GameState,
...
export function getSnakeEffects(
  state: GameState,
  index: PowerUpPlayerIndex
): SnakePowerUpEffects {
  return {
    ...
    amped: state.activePowerUps.some(
      (ap) =>
        ap.type === 'AMPLIFIER' &&
        ap.playerIndex === index &&
        (ap.chargesLeft ?? 0) > 0
    ),
  };
}

export function shouldPlayerMove(
  state: GameState,
  index: PowerUpPlayerIndex
): boolean {
...
export function hasSurgeDoubleStep(
  state: GameState,
  index: PowerUpPlayerIndex
): boolean {
  return (
...
function pushPowerUp(
  state: GameState,
  active: Omit<ActivePowerUp, 'expiresAtTick'> & { expiresAtTick?: number }
): void {
  ...
  switch (type) {
    ...
    case 'FREEZE': {
      const targets = isFfaMode(state)
        ? ([0, 1, 2, 3] as PowerUpPlayerIndex[]).filter(
            (i) => i !== pickerIndex
          )
  ...
  for (let i = state.powerUpItems.length - 1; i >= 0; i -= 1) {
    ...
    for (let pi = 0; pi < count; pi += 1) {
      ...
      state.powerUpRespawnCooldownTick =
        state.tickCount + POWERUP_RESPAWN_COOLDOWN_TICKS;
      return;
    }
  }
}
